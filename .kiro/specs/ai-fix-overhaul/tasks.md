...
- [ ] 1. Config File Reading — Make `set-provider` config actually work
  - [ ] 1.1 Create `sicario-cli/src/key_manager/config_file.rs` with `LocalConfig` struct and YAML parser
  ...
  - [ ] 1.2 Extend `resolve_endpoint()` in `key_manager/manager.rs` to read Config_File
  ...
  - [ ] 1.3 Extend `resolve_model()` in `key_manager/manager.rs` to read Config_File
  ...
  - [ ] 1.4 Extend `resolve_api_key()` in `key_manager/manager.rs` to read Config_File
  ...
  - [ ] 1.5 Update `cmd_config` in `main.rs` to use `resolve_*_with_source()` for `config show`
...
- [ ] 2. Dead Code Removal — Delete `cerebras_client.rs`
  - [ ] 2.1 Remove `sicario-cli/src/remediation/cerebras_client.rs`
...
- [ ] 3. Progress Indicator — Terminal spinner during LLM calls
  - [ ] 3.1 Create `sicario-cli/src/remediation/progress.rs` with `LlmProgressSpinner`
  ...
  - [ ] 3.2 Integrate spinner into `RemediationEngine::generate_fixed_content()`
...
- [ ] 4. Expanded Template Fixes — 6 new vulnerability types
  - [ ] 4.1 Create `sicario-cli/src/remediation/templates.rs` and extract existing template logic
  ...
  - [ ] 4.2 Implement path traversal template fix (CWE-22)
  ...
  - [ ] 4.3 Implement SSRF template fix (CWE-918)
  ...
  - [ ] 4.4 Implement insecure deserialization template fix (CWE-502)
  ...
  - [ ] 4.5 Implement hardcoded credentials template fix (CWE-798)
  ...
  - [ ] 4.6 Implement open redirect template fix (CWE-601) and XXE template fix (CWE-611)
...
- [ ] 5. Strict Syntax Validation — Reject unknown languages
  - [ ] 5.1 Add Ruby and PHP to the `Language` enum in `parser/mod.rs`
  ...
  - [ ] 5.2 Change `validate_syntax` to return `false` for unknown languages
...
- [ ] 6. Batch Mode — `--yes`/`--auto` flags
  - [ ] 6.1 Add `--yes` and `--auto` flags to `FixArgs` in `cli/fix.rs`
    - Add `#[arg(long, alias = "auto")] pub yes: bool` to `FixArgs`
    - _Requirements: 5.1, 5.2_

  - [ ] 6.2 Implement batch processing in `RemediationEngine`
  ...
  - [ ] 6.3 Update `cmd_fix` in `main.rs` to use batch mode
...
- [ ] 8. Cloud Provider Settings — Convex Backend (Phase 2)
  - [ ] 8.1 Add `providerSettings` table to `convex/convex/schema.ts`
  ...
  - [ ] 8.2 Create `convex/convex/providerSettings.ts` with CRUD mutations and queries
  ...
  - [ ] 8.3 Add HTTP routes to `convex/convex/http.ts`
...
- [ ] 9. Cloud Provider Settings — Dashboard UI (Phase 2)
  - [ ] 9.1 Create `ProviderTab` component in `SettingsPage.tsx`
  ...
  - [ ] 9.2 Implement form submission, test connection, and delete
...
- [ ] 10. Cloud Provider Settings — CLI Integration (Phase 2)
  - [ ] 10.1 Create `sicario-cli/src/key_manager/cloud_config.rs` with `CloudConfigFetcher`
  ...
  - [ ] 10.2 Extend resolution chain to include Cloud_Config
