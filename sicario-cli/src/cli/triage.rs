...
pub fn cmd_triage(args: TriageArgs) -> Result<()> {
    ...
    vulns.sort_by(|a, b| {
        let score_a = crate::confidence::scorer::compute_priority_score(a.severity, a.confidence_score, a.reachable, a.cloud_exposed, false);
        let score_b = crate::confidence::scorer::compute_priority_score(b.severity, b.confidence_score, b.reachable, b.cloud_exposed, false);
    ...
    println!("📋 Found {} findings. Starting interactive triage...", vulns.len());
    println!();
    ...
    for (i, vuln) in vulns.iter().enumerate() {
        let score = crate::confidence::scorer::compute_priority_score(vuln.severity, vuln.confidence_score, vuln.reachable, vuln.cloud_exposed, false);
        println!("Finding {} of {}", i + 1, vulns.len());
        println!("  Rule:      {}", vuln.rule_id);
        println!("  File:      {}:{}", vuln.file_path.display(), vuln.line);
        println!("  Severity:  {} (Priority Score: {:.1})", vuln.severity, score);
        ...
        loop {
            ...
            match input.as_str() {
                "i" | "ignore" => {
                    println!("📝 To ignore, add `// sicario-ignore: {}` above the line in the file.", vuln.rule_id);
                    println!();
                    break;
                }
                "s" | "skip" => {
